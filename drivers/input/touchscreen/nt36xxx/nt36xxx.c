...
struct nvt_ts_data *nvt_get_ts(void)
{
	return ts;
}
...
int32_t nvt_check_fw_reset_state(RST_COMPLETE_STATE check_reset_state)
{
	...
	while (1) {
		...
		if (unlikely(retry > 100)) {
			NVT_ERR("error, retry=%d, ", retry);
			NVT_ERR("buf[]=0x%02X,0x%02X,0x%02X,0x%02X,0x%02X\n",
...
{
	NVT_proc_entry = proc_create(DEVICE_NAME, 0444,
									NULL, &nvt_flash_fops);
...
static void nvt_parse_dt(struct device *dev)
{
	...
	ts->irq_gpio = of_get_named_gpio_flags(np,
		"novatek,irq-gpio", 0, &ts->irq_flags);
...
static void nvt_ts_work_func(struct work_struct *work)
{
	...
	ret = CTP_I2C_READ(ts->client,
		I2C_FW_Address, point_data, POINT_DATA_LEN + 1);
	if (ret < 0) {
...
void nvt_stop_crc_reboot(void)
{
	...
	if (((buf[1] == 0xFC) && (buf[2] == 0xFC) && (buf[3] == 0xFC)) ||
		...
		if (retry == 0)
			NVT_ERR("CRC auto reboot isn't able to be stopped!\n");
	}
	...
	const struct i2c_device_id *id)
{
	...
	input_set_abs_params(ts->input_dev,
		ABS_MT_PRESSURE, 0, TOUCH_FORCE_NUM, 0, 0);
	...
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0,
		ts->abs_y_max, 0, 0);
#if MT_PROTOCOL_B
	// no need to set ABS_MT_TRACKING_ID, input_mt_init_slots() already set
#else
	...
	wakeup_source_init(&gestrue_wakelock, "fts_tp");
#endif

	snprintf(ts->phys, sizeof("input/ts"), "input/ts");
