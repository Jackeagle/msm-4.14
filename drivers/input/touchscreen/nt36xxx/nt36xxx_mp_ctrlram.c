...
static int nvt_mp_buffer_init(void)
{
	...
	RecordResult_BufSize = IC_X_CFG_SIZE * IC_Y_CFG_SIZE + IC_KEY_CFG_SIZE;
	RawData_BufSize = (IC_X_CFG_SIZE * IC_Y_CFG_SIZE
				+ IC_KEY_CFG_SIZE) * sizeof(int32_t);
	RecordResult_Short = (uint8_t *)kzalloc(RecordResult_BufSize,
		GFP_KERNEL);
	...
	RecordResult_Short_Base = (uint8_t *)kzalloc(RecordResult_BufSize,
		GFP_KERNEL);
	...
	RecordResult_Open = (uint8_t *)kzalloc(RecordResult_BufSize,
		GFP_KERNEL);
	...
	RecordResult_FW_CC = (uint8_t *)kzalloc(RecordResult_BufSize,
		GFP_KERNEL);
	...
	RecordResult_FW_CC_Q = (uint8_t *)kzalloc(RecordResult_BufSize,
		GFP_KERNEL);
...
{
	struct nvt_ts_data *ts = nvt_get_ts();
	...
	if (ts->carrier_system) {
		...
		nvt_print_lmt_array(PS_Config_Lmt_Short_Diff_P, X_Channel,
			Y_Channel);
		printk("PS_Config_Lmt_Short_Diff_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_Short_Diff_N, X_Channel,
			Y_Channel);
		//---PS_Config_Lmt_Short_Base---
		printk("PS_Config_Lmt_Short_Base_P:\n");
		nvt_print_lmt_array(PS_Config_Lmt_Short_Base_P, X_Channel,
			Y_Channel);
		printk("PS_Config_Lmt_Short_Base_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_Short_Base_N, X_Channel,
			Y_Channel);
	} else {
	...
	if (ts->carrier_system) {
		...
		nvt_print_lmt_array(PS_Config_Lmt_FW_CC_I_P, X_Channel,
			Y_Channel);
		printk("PS_Config_Lmt_FW_CC_I_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_FW_CC_I_N, X_Channel,
			Y_Channel);
		//---PS_Config_Lmt_FW_CC_Q---
		printk("PS_Config_Lmt_FW_CC_Q_P:\n");
		nvt_print_lmt_array(PS_Config_Lmt_FW_CC_Q_P, X_Channel,
			Y_Channel);
		printk("PS_Config_Lmt_FW_CC_Q_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_FW_CC_Q_N, X_Channel,
			Y_Channel);
	} else {
		//---PS_Config_Lmt_FW_CC---
		printk("PS_Config_Lmt_FW_CC_P:\n");
		nvt_print_lmt_array(PS_Config_Lmt_FW_CC_P, X_Channel,
			Y_Channel);
		printk("PS_Config_Lmt_FW_CC_N:\n");
		nvt_print_lmt_array(PS_Config_Lmt_FW_CC_N, X_Channel,
			Y_Channel);
	}
	...
	uint8_t y_ch, const char *file_path, uint32_t offset)
{
	...
	for (y = 0; y < y_ch; y++) {
		for (x = 0; x < x_ch; x++) {
			...
			snprintf(fbufp + iArrayIndex * 7 + y * 2, "%5d, ",
				sizeof(rawdata[iArrayIndex]), rawdata[iArrayIndex]);
		}
		printk("\n");
	...
	for (k = 0; k < Key_Channel; k++) {
		...
		printk("%5d, ", rawdata[iArrayIndex]);
		snprintf(fbufp + keydata_output_offset + k * 7, "%5d, ",
			sizeof(rawdata[iArrayIndex]), rawdata[iArrayIndex]);
	}
	printk("\n");
	snprintf(fbufp + y_ch * x_ch * 7 + y_ch * 2 + Key_Channel * 7,
				sizeof("\r\n"), "\r\n");
#endif /* #if TOUCH_KEY_NUM > 0 */
...
static int32_t nvt_read_fw_noise(int32_t *xdata)
{
	...
	if (!ts->carrier_system) {
		...
		rawdata_diff_min_offset =
			Y_Channel * X_Channel * 7 + Y_Channel * 2 + Key_Channel * 7 + 2;
#else
...
static int32_t nvt_read_fw_open(int32_t *xdata)
{
...
#if TOUCH_KEY_NUM > 0
	rawdata_buf =
		(uint8_t *)kzalloc((IC_X_CFG_SIZE * IC_Y_CFG_SIZE + IC_KEY_CFG_SIZE) *
				2, GFP_KERNEL);
#else
	rawdata_buf =
		(uint8_t *)kzalloc(IC_X_CFG_SIZE * IC_Y_CFG_SIZE * 2, GFP_KERNEL);
#endif /* #if TOUCH_KEY_NUM > 0 */
	if (!rawdata_buf) {
	...
	for (y = 0; y < IC_Y_CFG_SIZE; y++) {
		...
		buf[0] = 0xFF;
		buf[1] =
			(uint8_t)(((raw_pipe_addr + y * IC_X_CFG_SIZE * 2) >> 16) & 0xFF);
		buf[2] =
			(uint8_t)(((raw_pipe_addr + y * IC_X_CFG_SIZE * 2) >> 8) & 0xFF);
		CTP_I2C_WRITE(ts->client, I2C_FW_Address, buf, 3);
...
static int32_t nvt_read_fw_short(int32_t *xdata)
{
...
#if TOUCH_KEY_NUM > 0
	rawdata_buf = (uint8_t *)kzalloc((X_Channel * Y_Channel + Key_Channel) *
					2, GFP_KERNEL);
#else
	...
	if (ts->carrier_system) {
		...
		for (y = 0; y < Y_Channel; y++) {
			...
			buf[0] = 0xFF;
			buf[1] =
				(uint8_t)(((raw_pipe_addr + y * X_Channel * 2) >> 16) & 0xFF);
			buf[2] =
				(uint8_t)(((raw_pipe_addr + y * X_Channel * 2) >> 8) & 0xFF);
			CTP_I2C_WRITE(ts->client, I2C_FW_Address, buf, 3);
			buf[0] = (uint8_t)((raw_pipe_addr + y * X_Channel * 2) & 0xFF);
			CTP_I2C_READ(ts->client, I2C_FW_Address, buf, X_Channel * 2 + 1);
			memcpy(rawdata_buf + y * X_Channel * 2, buf + 1, X_Channel * 2);
		...
		for (y = 0; y < Y_Channel; y++) {
			for (x = 0; x < X_Channel; x++) {
				...
				RawData_Short_Base[iArrayIndex] =
					(int16_t)(rawdata_buf[iArrayIndex * 2] + 256 *
						rawdata_buf[iArrayIndex * 2 + 1]);
			}
		}
#if TOUCH_KEY_NUM > 0
	...
	if (ts->carrier_system) {
		...
		// Save Rawdata to CSV file
		if (nvt_save_rawdata_to_csv(RawData_Short_Base, X_Channel, Y_Channel,
					SHORT_TEST_CSV_FILE, rawdata_short_base_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
	}
...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	if ((TestResult_Short == 0) || (TestResult_Short == 1)) {
		...
		if (ts->carrier_system) {
			...
			if (TestResult_Short_Diff == -1) {
				...
				print_selftest_result(m, TestResult_Short_Diff,
			...
			if (TestResult_Short_Base == -1) {
				...
				print_selftest_result(m, TestResult_Short_Base,
	...
	if ((TestResult_FW_Rawdata == 0) || (TestResult_FW_Rawdata == 1)) {
		 print_selftest_result(m,
			TestResult_FWMutual, RecordResult_FWMutual,
		...
		if (TestResult_FW_CC == -1) {
			if (ts->carrier_system) {
				if (TestResult_FW_CC_I == -1) {
					...
					print_selftest_result(m,
						TestResult_FW_CC_I, RecordResult_FW_CC_I,
						RawData_FW_CC_I, X_Channel, Y_Channel);
				}
				if (TestResult_FW_CC_Q == -1) {
					nvt_mp_seq_printf(m, "FW CC_Q");
					print_selftest_result(m,
						TestResult_FW_CC_Q, RecordResult_FW_CC_Q,
						RawData_FW_CC_Q, X_Channel, Y_Channel);
				}
			} else {
				nvt_mp_seq_printf(m, "FW CC");
				print_selftest_result(m,
					TestResult_FW_CC, RecordResult_FW_CC,
	...
	if ((TestResult_Noise == 0) || (TestResult_Noise == 1)) {
		print_selftest_result(m,
			TestResult_FW_DiffMax, RecordResult_FW_DiffMax,
		...
		if (TestResult_FW_DiffMin == -1) {
			...
			print_selftest_result(m,
				TestResult_FW_DiffMin, RecordResult_FW_DiffMin,
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
		...
		if (ts->carrier_system) {
			TestResult_FW_CC_I =
				RawDataTest_SinglePoint_Sub(RawData_FW_CC_I,
						RecordResult_FW_CC_I, X_Channel, Y_Channel,
						PS_Config_Lmt_FW_CC_I_P, PS_Config_Lmt_FW_CC_I_N);
			TestResult_FW_CC_Q = RawDataTest_SinglePoint_Sub(RawData_FW_CC_Q,
					RecordResult_FW_CC_Q, X_Channel, Y_Channel,
			...
			TestResult_FW_CC =
				RawDataTest_SinglePoint_Sub(RawData_FW_CC, RecordResult_FW_CC,
						X_Channel, Y_Channel, PS_Config_Lmt_FW_CC_P,
	...
	if (nvt_read_fw_short(RawData_Short) != 0) {
		...
		if (ts->carrier_system) {
			...
			TestResult_Short =
				RawDataTest_SinglePoint_Sub(RawData_Short, RecordResult_Short,
					X_Channel, Y_Channel, PS_Config_Lmt_Short_Rawdata_P,
...
return:
	n.a.
*******************************************************/
void nvt_mp_parse_array(struct device_node *np, const char *name,
...
void nvt_mp_parse_dt(struct device_node *root, const char *node_compatible)
{
	...
	/* MP Criteria */
	if (ts->carrier_system) {
		nvt_mp_parse_array(np, "PS_Config_Lmt_Short_Diff_P",
			PS_Config_Lmt_Short_Diff_P, X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array(np, "PS_Config_Lmt_Short_Diff_N",
			PS_Config_Lmt_Short_Diff_N, X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array(np, "PS_Config_Lmt_Short_Base_P",
			PS_Config_Lmt_Short_Base_P, X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array(np, "PS_Config_Lmt_Short_Base_N",
			PS_Config_Lmt_Short_Base_N, X_Channel * Y_Channel + Key_Channel);
	} else {
		nvt_mp_parse_array(np, "PS_Config_Lmt_Short_Rawdata_P",
		   PS_Config_Lmt_Short_Rawdata_P, X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array(np, "PS_Config_Lmt_Short_Rawdata_N",
		   PS_Config_Lmt_Short_Rawdata_N, X_Channel * Y_Channel + Key_Channel);
	}

	nvt_mp_parse_array(np, "PS_Config_Lmt_Open_Rawdata_P",
			PS_Config_Lmt_Open_Rawdata_P, X_Channel * Y_Channel + Key_Channel);

	nvt_mp_parse_array(np, "PS_Config_Lmt_Open_Rawdata_N",
			PS_Config_Lmt_Open_Rawdata_N, X_Channel * Y_Channel + Key_Channel);

	nvt_mp_parse_array(np, "PS_Config_Lmt_FW_Rawdata_P",
			PS_Config_Lmt_FW_Rawdata_P, X_Channel * Y_Channel + Key_Channel);

	nvt_mp_parse_array(np, "PS_Config_Lmt_FW_Rawdata_N",
			PS_Config_Lmt_FW_Rawdata_N, X_Channel * Y_Channel + Key_Channel);
	...
	if (ts->carrier_system) {
		nvt_mp_parse_array(np, "PS_Config_Lmt_FW_CC_I_P",
				PS_Config_Lmt_FW_CC_I_P, X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array(np, "PS_Config_Lmt_FW_CC_I_N",
				PS_Config_Lmt_FW_CC_I_N, X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array(np, "PS_Config_Lmt_FW_CC_Q_P",
				PS_Config_Lmt_FW_CC_Q_P, X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array(np, "PS_Config_Lmt_FW_CC_Q_N",
				PS_Config_Lmt_FW_CC_Q_N, X_Channel * Y_Channel + Key_Channel);
	}
