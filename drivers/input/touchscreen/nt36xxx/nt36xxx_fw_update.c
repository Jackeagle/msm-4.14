...
static struct firmware *fw_entry;
...
int32_t Check_FW_Ver(void)
{
	...
	// check if IC FW_VER + FW_VER_BAR equals 0xFF, need to update if not
	if ((buf[1] + buf[2]) != 0xFF) {
	...
	for (i = 0; i < BLOCK_64KB_NUM; i++) {
		if (fw_bin_size > (i * SIZE_64KB)) {
			...
			while (1) {
				...
				if (ret < 0) {
					NVT_ERR("Check 0xAA (Fast Read ");
					NVT_ERR("Command) error!!(%d)\n", ret);
					return ret;
				}
				if (buf[1] == 0xAA)
					break;
				...
				retry++;
				if (unlikely(retry > 5)) {
					NVT_ERR("Check 0xAA failed, retry=%d\n",
						retry);
					return -EPERM;
				}
			}
			...
			if (ret < 0) {
				NVT_ERR("Read Checksum (write addr high byte");
				NVT_ERR(" & middle byte) error!!(%d)\n", ret);
				return ret;
			}
			// Read Checksum
...
int32_t Init_BootLoader(void)
{
	...
	while (1) {
		...
		ret = CTP_I2C_READ(ts->client, I2C_HW_Address, buf, 2);
		if (ret < 0) {
			NVT_ERR("Check 0xAA (Inittial Flash Block)");
			NVT_ERR(" error!!(%d)\n", ret);
			return ret;
		}
		if (buf[1] == 0xAA)
...
int32_t Erase_Flash(void)
{
	...
	while (1) {
		...
		ret = CTP_I2C_READ(ts->client, I2C_HW_Address, buf, 2);
		if (ret < 0) {
			NVT_ERR("Check 0xAA (Write Enable for Write Status ");
			NVT_ERR(" Register) error!!(%d)\n", ret);
			return ret;
		}
		if (buf[1] == 0xAA)
			break;
		...
		if (unlikely(retry > 20)) {
			NVT_ERR("Check 0xAA (Write Enable for Write Status ");
			NVT_ERR("Register) error!! status=0x%02X\n", buf[1]);
			return -EPERM;
		}
	}
	...
	while (1) {
		...
		ret = CTP_I2C_READ(ts->client, I2C_HW_Address, buf, 2);
		if (ret < 0) {
			NVT_ERR("Check 0xAA (Write Status Register) ");
			NVT_ERR("error!!(%d)\n", ret);
			return ret;
		}
		if (buf[1] == 0xAA)
	...
	while (1) {
		...
		if (ret < 0) {
			NVT_ERR("Check 0xAA (Read Status for Write Status ");
			NVT_ERR("Register) error!!(%d)\n", ret);
			return ret;
		}
		...
		if (unlikely(retry > 100)) {
			NVT_ERR("Check 0xAA (Read Status for Write Status ");
			NVT_ERR("Register) failed, buf[1]=0x%02X, ", buf[1]);
			NVT_ERR("buf[2]=0x%02X, retry=%d\n", buf[2], retry);
			return -EPERM;
		}
	}
	...
	for (i = 0; i < count; i++) {
		...
		while (1) {
			...
			ret = CTP_I2C_READ(ts->client, I2C_HW_Address, buf, 2);
			if (ret < 0) {
				NVT_ERR("Check 0xAA (Write Enable) ");
				NVT_ERR("error!!(%d,%d)\n", ret, i);
				return ret;
			}
			if (buf[1] == 0xAA)
		...
		while (1) {
			...
			ret = CTP_I2C_READ(ts->client, I2C_HW_Address, buf, 2);
			if (ret < 0) {
				NVT_ERR("Check 0xAA (Sector Erase) ");
				NVT_ERR("error!!(%d,%d)\n", ret, i);
				return ret;
			}
			if (buf[1] == 0xAA)
		...
		while (1) {
			...
			ret = CTP_I2C_READ(ts->client, I2C_HW_Address, buf, 3);
			if (ret < 0) {
				NVT_ERR("Check 0xAA (Read Status) ");
				NVT_ERR("error!!(%d,%d)\n", ret, i);
				return ret;
			}
			if ((buf[1] == 0xAA) && (buf[2] == 0x00))
			...
			if (unlikely(retry > 100)) {
				...
				NVT_ERR("buf[1]=0x%02X, buf[2]=0x%02X, ",
					buf[1], buf[2]);
				NVT_ERR("retry=%d\n", retry);
				return -EPERM;
			}
...
int32_t Write_Flash(void)
{
	...
	XDATA_Addr = ts->mmap->RW_FLASH_DATA_ADDR;
	...
	for (i = 0; i < count; i++) {
		...
		for (k = 0; k < min(fw_entry->size - Flash_Address, (size_t)256);
			k++)
		...
		buf[5] = 0x00;
		buf[6] =
			min(fw_entry->size - Flash_Address, (size_t)256) - 1;
		buf[7] = tmpvalue;
	...
	for (i = 0; i < BLOCK_64KB_NUM; i++) {
		if (fw_bin_size > (i * SIZE_64KB)) {
			...
			while (1) {
				...
				retry++;
				if (unlikely(retry > 5)) {
					NVT_ERR("0xAA(Fast Read Command) fail");
					NVT_ERR("buf[1]=0x%02X, retry=%d\n",
						buf[1], retry);
					return -EPERM;
				}
...
void Boot_Update_Firmware(struct work_struct *work)
{
	...
	snprintf(firmware_name, sizeof(BOOT_UPDATE_FIRMWARE_NAME),
